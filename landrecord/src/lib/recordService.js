...
function mapFromDb(row) {
  return {
    ...
    fields: row.fields || [
      { id: 'owner', label: 'Owner Name', ocrValue: row.owner_name || row.ownerName, citizenValue: row.owner_name || row.ownerName, citizenAction: 'ACCEPTED', confidence: 99, adminDecision: 'PASS' },
      { id: 'khasra', label: 'Khasra Number', ocrValue: row.khasra_no || row.khasraNo, citizenValue: row.khasra_no || row.khasraNo, citizenAction: 'ACCEPTED', confidence: 96, adminDecision: 'PASS' },
      { id: 'area', label: 'Plot Area', ocrValue: row.area, citizenValue: row.area, citizenAction: 'ACCEPTED', confidence: 98, adminDecision: 'PASS' }
    ],
    lastAudited: row.last_audited || row.lastAudited || null,
    noticeSent: Boolean(row.notice_sent || row.noticeSent),
